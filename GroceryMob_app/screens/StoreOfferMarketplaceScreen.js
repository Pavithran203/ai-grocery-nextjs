...
import { ArrowLeft, Star, Truck, ShieldCheck, MapPin } from 'lucide-react-native';
...
export default function StoreOfferMarketplaceScreen({ navigation }) {
  const { t } = useLanguage();
  ...
  const comboOffers = useMemo(() => {
    ...
    nearbyStores.forEach(store => {
      ...
      allOffers = [...allOffers, ...storeOffers];
    });
    ...
    >
      ...
      <View style={styles.priceRow}>
        ...
        <View style={styles.actionButtons}>
          ...
          >
            <Text style={styles.buyBtnText}>{t('Add Combo')}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>{t('Smart Savings Zone')}</Text>
          <Text style={styles.headerSubtitle}>{t('Compare best store offers & combos')}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  trustBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFBF0', padding: 12, borderRadius: 12, marginBottom: 16, gap: 10, borderWidth: 1, borderColor: '#FFE8B2' },
  trustText: { flex: 1, fontSize: 12, color: '#D97706', fontWeight: '600', lineHeight: 18 },
  ...
  viewStoreBtn: { backgroundColor: COLORS.white, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: COLORS.gray[200] },
  viewStoreText: { color: COLORS.gray[600], fontSize: 13, fontWeight: '800' },
  buyBtn: { backgroundColor: '#F59E0B', paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12 },
