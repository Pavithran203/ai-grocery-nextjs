...
export default function MonthlyEssentialsScreen({ navigation }) {
  const { t } = useLanguage();
  ...
  const comboOffers = useMemo(() => {
    ...
    nearbyStores.forEach(store => {
      ...
      // Filter only monthly essential offers or all (for simplicity, we use the same combos)
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>{t('Monthly Savings')}</Text>
          <Text style={styles.headerSubtitle}>{t('Best deals on monthly groceries')}</Text>
        </View>
      </View>
