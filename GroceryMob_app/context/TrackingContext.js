...
export function TrackingProvider({ children }) {
  ...
  const startTracking = async (orderId) => {
    // Prevent duplicate tracking engine if same order is already tracking
    ...
    try {
      ...
      if (status === 'granted') {
        ...
        fUserLoc = { latitude: loc.coords.latitude, longitude: loc.coords.longitude };
      }
    } catch (e) {
    ...
    try {
      const url = `http://router.project-osrm.org/route/v1/driving/${fWhLoc.longitude},${fWhLoc.latitude};${fUserLoc.longitude},${fUserLoc.latitude}?overview=full&geometries=geojson`;
  ...
  const getProximityMessage = () => {
    if (currentStage === 'Order Placed') return 'Preparing your items...';
    if (currentStage === 'Packed') return 'Waiting for delivery partner...';
    if (currentStage === 'Delivered') return 'Order has been delivered!';
    
    if (distanceKm > 5) return 'Rider is on the way';
    if (distanceKm > 2) return 'Rider is getting closer';
    if (distanceKm > 0.5) return 'Rider is nearby';
    if (distanceKm > 0.1) return 'Rider is ' + Math.round(distanceKm * 1000) + 'm away';
    return 'Rider has almost arrived';
  };
