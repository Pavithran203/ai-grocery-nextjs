...
/**
 * Calculate a relevance score for a product based on user behavior.
 */
function scoreProduct(product, userProfile) {
  ...
  // Carted products/category boost
  ...
  // Search match — check if any search query matches product name or category
  for (const query of userProfile.searchedQueries) {
    if (product.name.toLowerCase().includes(query) || cat.toLowerCase().includes(query)) {
      score += WEIGHTS.SEARCHED;
      break;
    }
  }

  // Trending bonus (high review count)
  if (product.reviewCount && product.reviewCount > 200) {
    score += WEIGHTS.TRENDING_BONUS;
  }

  // Popular bonus (high rating)
...
/**
 * Get smart recommendations: 60% personalized + 40% trending/popular
 * @param {Array} allProducts - All available products
 * @param {Object} userProfile - User behavior data
 * @param {Array} cartItems - Current cart items (to exclude)
...
export function getSmartPicks(allProducts, userProfile, cartItems = [], count = 10) {
  ...
  // Trending picks (not already in personalized, sorted by reviewCount)
  const trending = scored
    .filter(p => !personalizedIds.has(p.id))
    .sort((a, b) => (b.reviewCount || 0) - (a.reviewCount || 0))
    .slice(0, trendingCount);

  // Mix them: alternate personalized and trending for variety
  const result = [];
  let pi = 0, ti = 0;
  while (result.length < count && (pi < personalized.length || ti < trending.length)) {
    ...
    if (result.length < count && pi < personalized.length && pi % 2 === 0) {
      // Add another personalized to maintain 60%
...
/**
 * Get reason label for why a product was recommended
 */
export function getRecommendationReason(product, userProfile) {
  ...
  for (const query of userProfile.searchedQueries) {
    if (product.name.toLowerCase().includes(query)) return `🔍 Related to "${query}"`;
  }
