...
export default function AddressesPage() {
  ...
  const { addresses, defaultAddressId, addAddress, removeAddress, updateAddress, setDefaultAddress } = useAddress();
  ...
  const isDefaultAddress = (addr) => {
    return addr.isDefault || addr.id === defaultAddressId || addr._id === defaultAddressId;
  };
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tighter mb-1">{t('profile.addresses.title')}</h1>
          <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">{t('profile.addresses.subtitle')}</p>
        </div>
        <button 
        ...
        >
          <Plus className="w-4 h-4" /> {t('profile.addresses.addNew')}
        </button>
      </div>
      ...
      {isAdding && (
        <div className="bg-white dark:bg-gray-900 rounded-[32px] border-2 border-emerald-500 shadow-xl p-8 relative animate-slideDown">
          ...
          <h2 className="text-2xl font-black text-gray-900 dark:text-white tracking-tighter mb-8 flex items-center gap-3">
            ...
            {editingId ? t('profile.addresses.editAddress') : t('profile.addresses.addNew')}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              />
              <span className="text-sm font-black text-gray-600 group-hover:text-emerald-600 transition-colors uppercase tracking-wider">{t('profile.addresses.setAsDefault')}</span>
            </label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {addresses.length === 0 && !isAdding ? (
          <div className="col-span-full py-20 text-center bg-white dark:bg-gray-900 rounded-[32px] border border-dashed border-gray-200 dark:border-gray-800">
            ...
            <h3 className="text-xl font-black text-gray-400 uppercase tracking-widest">{t('profile.addresses.noAddresses')}</h3>
            <p className="text-sm text-gray-300 font-bold mt-2">{t('profile.addresses.addFirst')}</p>
          </div>
        ) : (
          addresses.map((addr) => {
            const isDefault = isDefaultAddress(addr);
            return (
              <div key={addr.id || addr._id} className={`bg-white dark:bg-gray-900 rounded-[32px] border-2 transition-all p-6 group relative overflow-hidden ${
                isDefault ? 'border-emerald-500 shadow-md shadow-emerald-500/5' : 'border-gray-100 dark:border-gray-800 shadow-sm'
              }`}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${
                      isDefault ? 'bg-emerald-50 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400'
                    }`}>
                    ...
                    <div>
                      ...
                      {isDefault && <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-full uppercase tracking-widest">{t('profile.addresses.defaultLabel')}</span>}
                    </div>
                  </div>
